import Link from 'next/link'
import Image from 'next/image'

const GOLD = '#dcb068'
const RED = '#b8161d'
const BG = '#0d0705'

const services = [
  { href: '/shadeport-installation', label: 'Shadeport Installation' },
  { href: '/cantilever-shadeports', label: 'Cantilever Shadeports' },
  { href: '/carports', label: 'Carports' },
  { href: '/carport-installation', label: 'Carport Installation' },
  { href: '/parking-bay-shadeports', label: 'Parking Bay Shadeports' },
  { href: '/shadeport-repairs', label: 'Shadeport Repairs' },
  { href: '/shadeport-maintenance', label: 'Shadeport Maintenance' },
  { href: '/shadeport-recovering', label: 'Shadeport Re-Covering' },
]

const areas = [
  { name: 'Johannesburg', region: 'Gauteng' },
  { name: 'Pretoria', region: 'Gauteng' },
  { name: 'Sandton', region: 'Gauteng' },
  { name: 'Centurion', region: 'Gauteng' },
  { name: 'Polokwane', region: 'Limpopo' },
  { name: 'Tzaneen', region: 'Limpopo' },
]

export default function SiteFooter() {
  return (
    <footer className="text-white" style={{ background: BG }}>

      {/* ── Top bar ── */}
      <div className="border-b" style={{ borderColor: `${GOLD}26` }}>
        <div className="max-w-6xl mx-auto px-6 h-9 flex justify-between items-center">
          <p className="text-white/35 text-[11px] tracking-[0.12em]">
            Serving Gauteng &amp; Limpopo &middot; Shadeports &middot; Carports &middot; Repairs &amp; Maintenance
          </p>
          <div className="hidden sm:flex items-center gap-1.5 border px-2.5 py-1" style={{ borderColor: `${GOLD}33`, background: `${GOLD}14` }}>
            <span className="text-[8px]" style={{ color: GOLD }}>✦</span>
            <span className="text-[9px] font-bold tracking-[0.2em] uppercase" style={{ color: GOLD }}>
              Shadeports &amp; Carports Specialists
            </span>
          </div>
        </div>
      </div>

      {/* ── Main grid ── */}
      <div className="max-w-6xl mx-auto px-6 py-12 md:py-14">
        <div className="grid gap-10 md:grid-cols-[1.6fr_1fr_1fr]">

          {/* Brand column */}
          <div>
            <Link href="/" className="inline-flex mb-5" aria-label="AJT Enterprise, home">
              <Image
                src="/logo-horizontal.png"
                alt="AJT Enterprise"
                width={200}
                height={100}
                className="object-contain h-10 w-auto"
              />
            </Link>

            <p className="text-white/45 text-[12.5px] leading-[1.75] mb-5 max-w-[240px]">
              Shadeport and carport installation, repairs and maintenance across
              Gauteng and Limpopo. Built to last, backed by a team you can trust.
            </p>

            {/* Contact items */}
            <div className="flex flex-col gap-2.5">
              {/* Mobile */}
              <div className="flex items-center gap-2.5">
                <div className="w-7 h-7 border flex items-center justify-center shrink-0" style={{ borderColor: `${GOLD}33`, background: `${GOLD}1a` }}>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill={GOLD} aria-hidden="true">
                    <path d="M6.6 10.8c1.4 2.8 3.8 5.1 6.6 6.6l2.2-2.2c.3-.3.7-.4 1-.2 1.1.4 2.3.6 3.6.6.6 0 1 .4 1 1V20c0 .6-.4 1-1 1C10.6 21 3 13.4 3 4c0-.6.4-1 1-1h3.5c.6 0 1 .4 1 1 0 1.3.2 2.5.6 3.6.1.3 0 .7-.2 1L6.6 10.8z" />
                  </svg>
                </div>
                <a
                  href="tel:0658677363"
                  style={{ color: GOLD }}
                  className="hover:text-white transition-colors font-mono font-semibold text-[13px]"
                >
                  065 867 7363
                </a>
              </div>
              {/* Office */}
              <div className="flex items-center gap-2.5">
                <div className="w-7 h-7 bg-white/5 border border-white/10 flex items-center justify-center shrink-0">
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="rgba(255,255,255,0.3)" aria-hidden="true">
                    <path d="M6.6 10.8c1.4 2.8 3.8 5.1 6.6 6.6l2.2-2.2c.3-.3.7-.4 1-.2 1.1.4 2.3.6 3.6.6.6 0 1 .4 1 1V20c0 .6-.4 1-1 1C10.6 21 3 13.4 3 4c0-.6.4-1 1-1h3.5c.6 0 1 .4 1 1 0 1.3.2 2.5.6 3.6.1.3 0 .7-.2 1L6.6 10.8z" />
                  </svg>
                </div>
                <a
                  href="tel:0115685348"
                  className="text-white/55 hover:text-white transition-colors font-mono text-[12.5px]"
                >
                  011 568 5348
                </a>
              </div>
              {/* Email */}
              <div className="flex items-center gap-2.5">
                <div className="w-7 h-7 border flex items-center justify-center shrink-0" style={{ borderColor: `${GOLD}33`, background: `${GOLD}1a` }}>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill={GOLD} aria-hidden="true">
                    <path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z" />
                  </svg>
                </div>
                <a
                  href="mailto:Info@ajtenterprise.co.za"
                  className="text-white/55 hover:text-white transition-colors text-[12px]"
                >
                  Info@ajtenterprise.co.za
                </a>
              </div>
              {/* Location */}
              <div className="flex items-center gap-2.5">
                <div className="w-7 h-7 bg-white/5 border border-white/10 flex items-center justify-center shrink-0">
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="rgba(255,255,255,0.3)" aria-hidden="true">
                    <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z" />
                  </svg>
                </div>
                <p className="text-white/35 text-[12px]">Serving Gauteng &amp; Limpopo, South Africa</p>
              </div>
            </div>

            {/* Quote banner */}
            <div className="mt-6 flex items-center gap-3 border px-4 py-3.5" style={{ borderColor: `${GOLD}40`, background: `${GOLD}14` }}>
              <span className="relative flex h-2 w-2 shrink-0">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full opacity-60" style={{ background: GOLD }} />
                <span className="relative inline-flex rounded-full h-2 w-2" style={{ background: GOLD }} />
              </span>
              <div>
                <p className="text-[12px] font-bold tracking-wide" style={{ color: GOLD }}>Free On-Site Quotes</p>
                <p className="text-white/40 text-[11px] mt-0.5">Fast response across Gauteng &amp; Limpopo</p>
              </div>
            </div>
          </div>

          {/* Services column */}
          <div>
            <h3 className="flex items-center gap-2 text-[10px] font-bold tracking-[0.3em] uppercase text-white/30 mb-4">
              Our Services
              <span className="flex-1 h-px" style={{ background: `${GOLD}33` }} />
            </h3>
            <ul className="flex flex-col">
              {services.map((service) => (
                <li key={service.href}>
                  <Link
                    href={service.href}
                    className="group flex items-center gap-2 py-[7px] border-b border-white/[0.04] transition-colors hover:text-white text-white/50 text-[12.5px]"
                  >
                    <span className="w-1 h-1 shrink-0 transition-colors" style={{ background: `${GOLD}4d` }} />
                    {service.label}
                  </Link>
                </li>
              ))}
            </ul>
            <Link
              href="/services"
              style={{ color: GOLD }}
              className="flex items-center gap-1.5 mt-3 text-[11px] font-bold tracking-[0.2em] uppercase hover:text-white transition-colors"
            >
              All Services <span aria-hidden="true">→</span>
            </Link>
          </div>

          {/* Areas column */}
          <div>
            <h3 className="flex items-center gap-2 text-[10px] font-bold tracking-[0.3em] uppercase text-white/30 mb-4">
              Areas We Serve
              <span className="flex-1 h-px" style={{ background: `${GOLD}33` }} />
            </h3>
            <ul className="flex flex-col">
              {areas.map((area) => (
                <li key={area.name}>
                  <span className="flex items-center gap-2 py-[7px] border-b border-white/[0.04] text-white/50 text-[12.5px]">
                    <span className="w-1 h-1 shrink-0" style={{ background: `${GOLD}4d` }} />
                    {area.name}
                    <span className="text-white/25 text-[10px]">· {area.region}</span>
                  </span>
                </li>
              ))}
            </ul>
            <Link
              href="/contact"
              style={{ color: GOLD }}
              className="flex items-center gap-1.5 mt-3 text-[11px] font-bold tracking-[0.2em] uppercase hover:text-white transition-colors"
            >
              Don&apos;t See Your Area? <span aria-hidden="true">→</span>
            </Link>
          </div>

        </div>
      </div>

      {/* ── Gold divider ── */}
      <div className="max-w-6xl mx-auto px-6">
        <div className="h-px" style={{ background: `linear-gradient(to right, transparent, ${GOLD}40, transparent)` }} />
      </div>

      {/* ── Bottom bar ── */}
      <div className="max-w-6xl mx-auto px-6 py-4 flex flex-col md:flex-row justify-between items-center gap-3">
        <p className="text-[11px] text-white/25">
          &copy; {new Date().getFullYear()} AJT Enterprise. All rights reserved.
        </p>
        <div className="flex gap-5">
          <Link href="/privacy-policy" className="text-[11px] text-white/30 hover:text-white tracking-wide transition-colors">
            Privacy Policy
          </Link>
          <Link href="/terms" className="text-[11px] text-white/30 hover:text-white tracking-wide transition-colors">
            Terms of Service
          </Link>
        </div>
      </div>

    </footer>
  )
}
